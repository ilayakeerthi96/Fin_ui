...
var _destroyRef, _activatedRoute, _router, _titleService, _colorModeService, _authService;
var _AppComponent = class _AppComponent {
  constructor(router) {
    ...
    this.router = router;
    this.title = "Supplier Management System";
    ...
    __privateSet(this, _authService, inject(AuthService));
  ...
  ngOnInit() {
    ...
    if (token) {
      let currentUrl = "";
      try {
        currentUrl = this.router && this.router.url ? this.router.url : "";
      } catch {
      }
    ...
    if (status !== null) {
      ...
      __privateGet(this, _authService).setLoginStatus(this.loginStatus);
    } else {
      this.loginStatus = false;
      localStorage.setItem("loginStatus", "false");
    }
    __privateGet(this, _authService).loginStatus$.pipe(takeUntilDestroyed(__privateGet(this, _destroyRef))).subscribe((status2) => {
      this.loginStatus = status2;
      localStorage.setItem("loginStatus", String(status2));
    });
  }
};
...
var _AuthInterceptor = class _AuthInterceptor {
  ...
  intercept(req, next) {
    ...
    return next.handle(authReq).pipe(catchError((error) => {
      ...
      if (error.status === 401) {
        console.error("\u{1F510} Authentication failed - redirecting to login");
        this.handleAuthError();
      } else if (error.status === 403) {
        this.messageService.showMessage("error", "Access Denied", "You do not have permission to perform this action");
      } else if (error.status === 0) {
        this.messageService.showMessage("error", "Connection Error", "Unable to connect to server. Please check your connection.");
      }
  ...
  handleAuthError() {
    localStorage.clear();
    this.messageService.showMessage("warning", "Session Expired", "Your session has expired. Please login again.");
    setTimeout(() => {
...
_AuthInterceptor.\u0275fac = function AuthInterceptor_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _AuthInterceptor)(\u0275\u0275inject(Router), \u0275\u0275inject(MessageService));
};
